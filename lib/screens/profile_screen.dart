...
class ProfileScreen extends StatelessWidget {
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            ...
            const Expanded(child: SubmittedBloodRequests(activeOnly: true)),
          ],
        ),
      ),
    );
  ...
  Widget _bloodIcon() {
    final bloodType = Hive.box(ConfigBox.key).get(
      ConfigBox.bloodType,
  ...
  Widget _headerImage(String? url) => Stack(
    ...
    children: [
      SizedBox(
        ...
        height: curveHeight,
        child: CustomPaint(painter: _ProfileCurvePainter()),
      ),
      Hero(
        tag: 'profilePicHero',
        child: Container(
...
class _ProfileCurvePainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    final leftCurveControlPoint = Offset(
      size.width * 0.2,
      size.height - avatarRadius * 0.8,
    );
    final rightCurveControlPoint = Offset(
      size.width - leftCurveControlPoint.dx,
      size.height - avatarRadius * 0.8,
    );

    final avatarLeftPoint = Offset(
      size.width * 0.5 - avatarRadius + 5,
      size.height * 0.5,
    );
    ...
    final avatarTopPoint = Offset(
      size.width / 2,
      size.height / 2 - avatarRadius,
    );
    ...
    final path = Path()
      ...
      ..arcToPoint(
        avatarTopPoint,
        radius: const Radius.circular(avatarRadius),
      )
